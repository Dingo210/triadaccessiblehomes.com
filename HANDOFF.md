...
│
├── scripts/
│   ├── seed.ts                    # seeds the 22 Triad businesses (idempotent upsert)
...
Idempotent `upsert` seed of the 22 Triad businesses. `slugify(name)` becomes the row `id`.
...
guide & city slugs, all 22 listings) and `robots.ts` (disallows `/admin` and `/api/`). JSON-LD on
...
# 4. Seed the 22 Triad businesses (idempotent — safe to re-run)
yarn prisma db seed
...
- **22 seeded businesses** — real Triad-area providers. (Blue Ridge Builders of the Triad added 2026-09-11; Comer Shuford LLC added 2026-09-14.) `email` is optional in the seed and set only for providers that already publish one. Their `website` links point to the
